...
- `images.*` governs thumbnail/full-size resizing and JPEG quality; `scaleMethod` accepts `speed`, `balanced`, `quality`, `ultra_quality`, or `automatic`. Widths must be positive and `jpegQuality` must be between `0.0` and `1.0`. JPEG quality applies only to `.jpg` and `.jpeg` outputs.
...
## Image Handling
Markdown image syntax (`![alt](photo.png)`) renders responsive figures: Pologen resolves JPEG, PNG, and GIF input relative to the post folder, emits `photo-full.png` and `photo-thumb.png` using the source extension and matching image format, and injects HTML that opens the full asset. Unsupported formats such as WebP are left as ordinary Markdown image references without generated thumbnails. Unchanged image sources are reused using SHA-256 fingerprints stored in `meta.toml`. Pologen does not delete artifacts generated by earlier versions.
